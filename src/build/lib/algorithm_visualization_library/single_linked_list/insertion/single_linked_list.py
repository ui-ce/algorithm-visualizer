...
class SingleLinkedList:
    ...
    def generate_latex_for_state(self):
        ...
        for i in range(node_counter - 2):
            latex_code += (f"    \\path[*->] let \\p1 = (N{i + 1}.three), \\p2 = (N{i + 1}.center) in (\\x1,\\y2) edge "
                           f"[bend left] ($(N{i + 2}.one)+(0.1,0.06)$);\n")
    ...
    def generate_combined_latex(self, before_code, after_code):
        ...
        latex_code += (r"\tikzset{list/.style={very thick, rectangle split, rectangle split parts=3, draw, rectangle "
                       r"split horizontal, minimum size=18pt, inner sep=5pt, text=black, rectangle split part fill={"
                       r"blue!20, red!20, green!20}}, ->, start chain=going right, very thick}") + "\n"
